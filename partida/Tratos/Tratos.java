...
public class Tratos {
    ...
    public void proponerTrato(Jugador jugadorOferta, ArrayList<Jugador> jugadores, Tablero tablero) {
        ...
        Juego.consola.print(
                "Ingrese los nombres de las propiedades que el jugador espera recibir, para terminar escribe \"fin\": ");
        while (true) {
        ...
        Trato nuevoTrato = new Trato(id, jugadorOferta, jugadorAcepta, cantidadOferta, cantidadAcepta,
                propiedadesOferta, propiedadesAcepta);
    ...
    public void aceptarTrato(Jugador jugador) {
        ...
        do {
            input = Juego.consola
                    .read("Introduce el número del trato que quieres aceptar (? para listarlos / x para salir): "); // Leer
                                                                                                                    // la
                                                                                                                    // entrada
                                                                                                                    // como
                                                                                                                    // String
                ...
                for (Trato trato : tratos) {
                    if (trato.getId() == idAcepta) {
                        if (!trato.getJugador_acepta().equals(jugador)) {
                            Juego.consola.error("El trato " + trato.getId() + " es para el jugador "
                                    + trato.getJugador_acepta().getNombre());
                            break;
                        }
    ...
    public void rechazarTrato(Jugador jugador) {
        ...
        do {
            input = Juego.consola
                    .read("Introduce el número del trato que quieres rechazar (? para listarlos / x para salir): "); // Leer
                                                                                                                     // la
                                                                                                                     // entrada
                                                                                                                     // como
                                                                                                                     // String
                ...
                for (Trato trato : tratos) {
                    if (trato.getId() == idRechazo) {
                        if (!(trato.getJugador_oferta().equals(jugador) || trato.getJugador_acepta().equals(jugador))) {
                            Juego.consola.print(
                                    "El jugador " + jugador.getNombre() + " no forma parte del trato " + trato.getId());
                        }
                        rechazado = true;
                        tratos.remove(trato);
    ...
    private void ejecucionTrato(Trato trato) throws TratosException {
        for (Propiedad propiedad : trato.getPropiedades_oferta()) {
            if (!trato.getJugador_oferta().getPropiedades().contains(propiedad)) {
                throw new TratosException("El jugador " + trato.getJugador_oferta().getNombre()
                        + " no posee la propiedad " + propiedad.getNombre());
            }
        }

        for (Propiedad propiedad : trato.getPropiedades_acepta()) {
            if (!trato.getJugador_acepta().getPropiedades().contains(propiedad)) {
                throw new TratosException("El jugador " + trato.getJugador_acepta().getNombre()
                        + " no posee la propiedad " + propiedad.getNombre());
            }
        }
        ...
        if (trato.getJugador_oferta().getFortuna() < trato.getCantidad_oferta()) {
            throw new TratosException("El jugador " + trato.getJugador_oferta().getNombre()
                    + " no tiene el dinero suficiente para el trato (" + trato.getCantidad_oferta() + ")");
        }

        if (trato.getJugador_acepta().getFortuna() < trato.getCantidad_acepta()) {
            throw new TratosException(("El jugador " + trato.getJugador_acepta().getNombre()
                    + " no tiene el dinero suficiente para el trato (" + trato.getCantidad_acepta() + ")"));
        }
