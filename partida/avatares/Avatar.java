...
public abstract class Avatar {
    ...
    public int turno = 0; // Índice correspondiente a la posición en el arrayList del jugador (y el
    ...
    private void moverAvatar(ArrayList<ArrayList<Casilla>> casillas, int valorTirada, Tablero tablero) {
        if (!this.getJugador().getEnCarcel()) {
            ...
            if (valorTirada >= 0)
                this.getJugador().cobrarPasoPorSalida(casillaActual, nuevaCasilla);
            else
                this.getJugador().PasoPorSalidaInverso(casillaActual, nuevaCasilla);

            lugar.eliminarAvatar(this);
            this.lugar = nuevaCasilla;
            nuevaCasilla.anhadirAvatar(this);
            Juego.consola.print("El jugador " + this.jugador.getNombre() + " se mueve " + valorTirada
    ...
    public void moverEnBasico(ArrayList<ArrayList<Casilla>> casillas, int valorTirada, Jugador banca, Tablero tablero,
    ...
    // public abstract boolean moverEnAvanzado(ArrayList<ArrayList<Casilla>>
    // casillas, int valorTirada, Jugador banca, Tablero tablero, ArrayList<Jugador>
    // jugadores);

    public abstract int moverEnAvanzado(int resultadoTotal, int faltaPorMover, ArrayList<ArrayList<Casilla>> casillas,
